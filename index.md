...
<ul>
  {% for post in site.posts %}
    {% if post.tags contains 'song' %}
      <li>
        <h2 ><a class="shrink-ban" style="color: #BB0000; letter-spacing: -1px; font-weight: 300; text-decoration: underline white;" href="{{ post.url }}" onclick="this.parentElement.classList.toggle('active')>{{ post.date | date: "%B %d, %Y"}} - {{ post.title }}</a>
        </h2>
